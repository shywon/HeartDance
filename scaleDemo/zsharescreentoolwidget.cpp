...
ZShareScreenToolWidget::ZShareScreenToolWidget(QWidget *parent)
	: ZMoveableWidget(parent)
...
void ZShareScreenToolWidget::setOnlineStudent(int cur, int total)
{
	m_onlineStudentLbl->setText(QString("在线学生:%1/%2").arg(cur).arg(total));
}

void ZShareScreenToolWidget::start()
{
	m_shareTimeWidget->startTimer();
}

void ZShareScreenToolWidget::onClicked()
{
...
void ZShareScreenToolWidget::springback()
{
	m_state = m_state == ToolState_Expading ? ToolState_Shrink : ToolState_Expading;
	initSize();
}

void ZShareScreenToolWidget::initUi()
{
	...
	//
	m_shareTimeWidget = new ZScreenShareTimeWidget(m_baseWidget);
...
void ZShareScreenToolWidget::initConnect()
{
	//初始化学生数量
	setOnlineStudent(0, 0);

	connect(this, &ZShareScreenToolWidget::sigClickLogo, this, &ZShareScreenToolWidget::springback);
	connect(m_finishShareBtn, &QPushButton::clicked, this, &ZShareScreenToolWidget::onClicked);

}

void ZShareScreenToolWidget::initSize()
{
	switch (m_state)
	{
	case ZShareScreenToolWidget::ToolState_Expading:
	{
		m_shrinkBtn->show();
		m_expandBtn->hide();
	...
	break;
	case ZShareScreenToolWidget::ToolState_Shrink:
	{
		m_shrinkBtn->hide();
		m_expandBtn->show();
